            ...
            with psy.connect(conn_str) as conn:
                with conn.cursor() as cur:
                    ...
                    if separar_herramientas:
                        ...
                        herramientas_summary = {}
                        for fila in rows:
                            sede = fila[0]  # primera columna es la sede
                            if sede not in herramientas_summary:
                                herramientas_summary[sede] = []
                            herramientas_summary[sede].append((
                                fila[1],  # herramienta
                                fila[2],  # total_tiempo
                                fila[3],  # total_prototipos
                                fila[4],  # total_proyectos
                            ))
                    else:
                        herramientas_summary = rows

                    if herramientas_summary:
                        print("Columnas devueltas:", len(herramientas_summary[0]))
                    else:
                        print("Columnas devueltas: 0 (sin datos)")'''
